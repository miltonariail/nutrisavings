...
$Path = "C:\Native Data\Nutrisavings\tracking_files\"
$DateStamp = "03042019"              #= ((Get-Date).ToUniversalTime()).ToString("MMddyyy")
...
Function Update-NuSure_Membership() {
    ...
    $excel.Run("NuSu_AddMembers")
    #$workbook.save()
    $workbook.close()
    $excel.quit()
    Write-Host "NuSure Membership File Updated"
}
